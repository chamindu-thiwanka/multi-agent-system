...
PURPOSE:
    Defines the three core API endpoints required by the assignment:
    POST /query   - starts the agent pipeline, waits for human checkpoint
    POST /approve - resumes after human approval
    GET  /trace   - returns execution trace for any session

DESIGN DECISION: POLLING INSTEAD OF SSE
    The original streaming (SSE) approach had threading issues that
    prevented events from reaching the frontend reliably.
    
    We now use a simpler polling model:
    1. Frontend calls POST /query
    2. Backend runs the full pipeline until the human checkpoint
    3. Backend returns draft + trace when ready (after 2-10 minutes)
    4. Frontend shows draft + approve button
    5. Frontend calls POST /approve
    6. Backend resumes and returns final answer
    
    This is completely reliable and simpler to debug.
    The trace still shows all steps - just all at once when ready.
"""
...
class QueryRequest(BaseModel):
    ...
    query: str = Field(
        ...
        min_length=3,
        max_length=1000,
        example="What are recent AI approaches for detecting Alzheimers disease?"
    )
    
    session_id: Optional[str] = Field(
        default=None,
        description="Session ID for continuing a conversation. New session created if not provided."
    )
...
class ApproveRequest(BaseModel):
    ...
    approved: bool = Field(
        default=True,
        description="True to approve the draft"
    )
...
# ================================================================
# SYNC GRAPH RUNNERS (called in thread pool)
# ================================================================

def _run_graph_to_checkpoint(state: dict, config: dict) -> None:
    """
    Runs the graph until it pauses at the human checkpoint.
    
    This function is SYNCHRONOUS (blocking).
    It runs in a thread pool via run_in_executor so it does not
    block FastAPI's async event loop.
    
    graph.stream() yields one event per node execution.
    We consume all events until the stream ends (graph paused).
    We don't need to process the events here - LangGraph saves
    all state to the SQLite checkpointer automatically.
    After this function returns, we read the final state from
    the checkpointer using graph.get_state().
    """
    graph = get_graph()
    
    try:
        for event in graph.stream(state, config):
            # Log each node as it executes
            for node_name in event.keys():
                if node_name != "__interrupt__":
                    logger.info(f"[Graph Runner] Node completed: {node_name}")
    except Exception as e:
        logger.error(f"[Graph Runner] Stream error: {e}")
        raise


def _resume_graph_after_approval(config: dict) -> None:
    """
    Resumes the graph after human approval.
    
    Passing None as state tells LangGraph:
    "don't start over, resume from the saved checkpoint."
    LangGraph loads the state saved during the pause
    and continues from human_checkpoint_node.
    """
    graph = get_graph()
    ...
    try:
        for event in graph.stream(None, config):
            for node_name in event.keys():
                if node_name != "__interrupt__":
                    logger.info(f"[Graph Resume] Node completed: {node_name}")
    except Exception as e:
        logger.error(f"[Graph Resume] Stream error: {e}")
        raise
...
@router.post(
    ...
    summary="Submit a research query",
    description=(
        "Starts the multi-agent pipeline. Runs Planner, Tools, Draft, "
        "and Verifier nodes. Pauses at human checkpoint and returns the "
        "draft answer and execution trace. Takes 2-10 minutes with local LLM."
    ),
)
async def submit_query(request: QueryRequest):
    """
    Starts and runs the multi-agent pipeline until the human checkpoint.
    
    WHAT HAPPENS:
    1. Create initial graph state from the query
    2. Load any relevant long-term memory context
    3. Run the graph in a thread pool (non-blocking)
    4. Graph executes: planner → tools → draft → verifier
    5. Graph pauses before human_checkpoint_node
    6. We read the state from the SQLite checkpointer
    7. Return draft answer + trace to frontend
    
    The frontend then shows the draft and an Approve button.
    Call POST /approve with the session_id to get the final answer.
    
    NOTE: This endpoint takes 2-10 minutes to respond because
    it waits for the local LLM to complete all nodes.
    The frontend should show a loading indicator.
    """
    
    logger.info(f"[API] POST /query received: '{request.query[:80]}'")
    ...
    try:
        ...
        # LangGraph config - thread_id identifies this run in the checkpointer
        # Every operation for this session uses the same thread_id
        # so LangGraph can find the saved state
        config = {"configurable": {"thread_id": session_id}}
        
        logger.info(f"[API] Starting pipeline for session {session_id[:8]}")
        
        # Run graph in a thread pool
        # run_in_executor(None, fn, *args) = run fn(*args) in default thread pool
        # await = wait for it without blocking other FastAPI requests
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None,
            _run_graph_to_checkpoint,
            state,
            config,
        )
        
        # Read the final state from the SQLite checkpointer
        # The graph saved its state here before pausing
        graph = get_graph()
        checkpoint = graph.get_state(config)
        
        if checkpoint is None or checkpoint.values is None:
            raise ValueError("Graph state not found after execution")
        
        state_dict = dict(checkpoint.values)
        display = format_state_for_display(state_dict)
        
        logger.info(
            f"[API] Pipeline complete: session={session_id[:8]} "
            f"status={display['status']} "
            f"tools={display.get('tools_used', [])}"
        )
...
@router.post(
    "/approve",
    response_model=ApproveResponse,
    summary="Approve the draft answer",
    description=(
        "Resumes the paused pipeline after human review. "
        "Runs human_checkpoint_node and final_output_node. "
        "Returns the polished final answer."
    ),
)
async def approve_draft(request: ApproveRequest):
    """
    ...
    WHAT HAPPENS:
    1. Load the paused state from SQLite checkpointer (by session_id)
    2. Inject human_approved=True into the state
    3. Resume graph execution: human_checkpoint_node → final_output_node
    4. Return the final answer
    
    The session_id MUST match the one from /query.
    Without the correct session_id, LangGraph can't find the
    saved state and can't resume.
    """
    ...
    try:
        ...
        # Verify this session exists in the checkpointer
        current_state = graph.get_state(config)
        
        if current_state is None or not dict(current_state.values):
            raise HTTPException(
                status_code=404,
                detail=(
                    f"Session '{request.session_id}' not found. "
                    f"Please submit a query first."
                )
            )
        
        # Inject the human approval into the saved state
        # update_state() modifies the checkpoint for this thread_id
        # The next stream() call sees human_approved=True
        graph.update_state(
            config,
            {
                "human_approved": request.approved,
        ...
        logger.info(f"[API] State updated with approval, resuming graph")
        
        # Resume graph in thread pool
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(
            None,
            _resume_graph_after_approval,
            config,
        )
        
        # Read the final state
        final_checkpoint = graph.get_state(config)
        state_dict = dict(final_checkpoint.values)
        display = format_state_for_display(state_dict)
...
@router.get(
    ...
    description=(
        "Returns the full execution trace showing which tools were called "
        "and what decisions were made. Use session_id from /query response."
    ),
)
async def get_trace(session_id: str):
    """
    Returns the execution trace for a given session.
    
    The trace is an ordered list of strings:
    ["Planner: selected tools [rag, internet_search]",
     "RAG Tool: retrieved 5 articles from PubMed",
     ...]
    
    Used by the frontend to display what the agents did.
    ...
    try:
        ...
        if state is None or not dict(state.values):
            raise HTTPException(
                status_code=404,
                detail=f"Session '{session_id}' not found"
            )
...
# ================================================================
# ENDPOINT 4: GET /status/{session_id}
# ================================================================
...
async def get_status(session_id: str):
    """Returns full current state including draft and final answer."""
    ...
    try:
        ...
        return format_state_for_display(dict(state.values))
