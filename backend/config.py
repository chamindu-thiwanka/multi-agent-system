...
#         # __file__ is config.py's path, parent is backend/, parent.parent is root
...
def get_llm():
    ...
    WHY IMPORT INSIDE THE IF BLOCKS?
        ...
        This means you don't need cloud provider packages installed
        if you're only using local models.
...
def get_embeddings():
    ...
    HOW VECTOR SIMILARITY WORKS:
        ...
        "Closest" is measured using cosine similarity (angle between vectors).
    ...
    if settings.llm_provider == "ollama":
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # So we need a flexible match that handles both formats
                models = [m["name"] for m in response.json().get("models", [])]

                def model_exists(model_name, available):
                    """
                    Check if model_name matches any model in available list.

                    Handles these cases:
                    - Exact match:    "llama3:latest" == "llama3:latest"
                    - Tag mismatch:   "llama3" matches "llama3:latest"
                                      (splits on : and compares base name)

                    This is needed because Ollama always stores models with
                    tags (e.g. :latest) but users often type just the name.
                    """
                    return any(
                        model_name == m or                # exact match
                        model_name == m.split(":")[0]     # match base name without tag
                        for m in available
                    )
                ...
                if not model_exists(settings.embedding_model_name, models):
                    errors.append(
                        f"Embedding model '{settings.embedding_model_name}' not found. "
                        f"Available models: {models}. "
                        f"Run: ollama pull {settings.embedding_model_name}"
                    )
            ...
            # This means Ollama is not running at all
            errors.append(
                f"Cannot connect to Ollama at {settings.llm_base_url}. "
            ...
            # Ollama is running but very slow (maybe loading a model)
            warnings.append(
                f"Ollama at {settings.llm_base_url} is slow to respond. "
                f"It may be loading a model. Try again in a moment."
            )
